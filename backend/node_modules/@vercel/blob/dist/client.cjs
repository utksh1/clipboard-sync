...
var put = _chunkQTTJNF6Bcjs.createPutMethod.call(void 0, {
  ...
  extraChecks(options) {
    if (typeof window === "undefined") {
      throw new (0, _chunkQTTJNF6Bcjs.BlobError)(
        "client/`put` must be called from a client environment"
      );
    }
    if (!options.token.startsWith("vercel_blob_client_")) {
      throw new (0, _chunkQTTJNF6Bcjs.BlobError)("client/`put` must be called with a client token");
    }
    if (
...
var upload = _chunkQTTJNF6Bcjs.createPutMethod.call(void 0, {
  allowedOptions: ["contentType"],
  extraChecks(options) {
    if (typeof window === "undefined") {
      throw new (0, _chunkQTTJNF6Bcjs.BlobError)(
    ...
    if (options.handleUploadUrl === void 0) {
      throw new (0, _chunkQTTJNF6Bcjs.BlobError)("Missing `handleUploadUrl` parameter");
    }
    if (
      // @ts-expect-error -- Runtime check for DX.
      options.addRandomSuffix !== void 0 || // @ts-expect-error -- Runtime check for DX.
...
async function handleUpload({
  ...
  var _a, _b, _c, _d;
  const resolvedToken = _chunkQTTJNF6Bcjs.getTokenFromOptionsOrEnv.call(void 0, { token });
  ...
  switch (type) {
    ...
    case "blob.upload-completed": {
      ...
      if (!signature) {
        throw new (0, _chunkQTTJNF6Bcjs.BlobError)("Missing callback signature");
      }
      ...
      if (!isVerified) {
        throw new (0, _chunkQTTJNF6Bcjs.BlobError)("Invalid callback signature");
      }
      await onUploadCompleted(body.payload);
      return { type, response: "ok" };
    }
    default:
      throw new (0, _chunkQTTJNF6Bcjs.BlobError)("Invalid event type");
  }
}
async function retrieveClientToken(options) {
  ...
  if (!res.ok) {
    throw new (0, _chunkQTTJNF6Bcjs.BlobError)("Failed to  retrieve the client token");
  }
  try {
...
async function generateClientTokenFromReadWriteToken({
  ...
  const readWriteToken = _chunkQTTJNF6Bcjs.getTokenFromOptionsOrEnv.call(void 0, { token });
  ...
  if (!securedKey) {
    throw new (0, _chunkQTTJNF6Bcjs.BlobError)("Unable to sign client token");
  }
